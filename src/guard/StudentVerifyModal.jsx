...
export default function StudentVerifyModal({
  ...
  const isInCampus = record.std_status === "In" || !record.std_status;
  const isOutstation =
    (record.outpass_type || record.type || "").toLowerCase() === "outstation";
  const isHome =
    (record.outpass_type || record.type || "").toLowerCase() === "home";

  const handleConfirmAction = async () => {
    const actionType = isInCampus ? "exit" : "enter";
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-3 sm:p-4 animate-in fade-in zoom-in duration-200">
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border border-gray-100 flex flex-col relative max-h-[90vh]">
        {/* Top Gradient Header */}
        <div className="bg-gradient-to-r from-[#6d0f16] to-[#8a1822] text-white p-6 pb-5 relative">
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-3 text-center">
              ...
              <p className="text-xs font-bold text-[#6d0f16] mt-0.5 truncate">{record.hostel || "Assigned Hostel"}</p>
            </div>
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-3 text-center">
            ...
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-3 text-center">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Pass ID</p>
              <p className="text-[11px] font-mono font-bold text-gray-600 mt-0.5 truncate">
                OP-{String(targetId).substring(0, 8)}
              </p>
            </div>
          </div>

          {/* Visit & Departure/Return Details */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="bg-gray-50 border border-gray-100 rounded-xl p-3 flex items-center justify-between">
              ...
              {record.phone && (
                <a
                  ...
                  className="px-3 py-1 bg-white border border-gray-200 text-gray-700 rounded-lg text-xs font-bold hover:bg-gray-100 transition"
                >
                  Call
                </a>
              )}
            ...
            <div className="bg-gray-50 border border-gray-100 rounded-xl p-3 flex items-center justify-between">
              ...
              {(record.parent_contact || record.parent_number) && (
                <a
                  href={`tel:${record.parent_contact || record.parent_number}`}
                  className="px-3 py-1 bg-white border border-gray-200 text-[#6d0f16] rounded-lg text-xs font-bold hover:bg-gray-100 transition"
                >
                  Call Parent
                </a>
              )}
          ...
          <div>
            ...
            <input
              type="text"
              placeholder={isInCampus ? "e.g. Left with bag on motorcycle" : "e.g. Returned on time safely"}
              value={remark}
        ...
        <div className="p-5 border-t border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          >
            ...
            {isInCampus ? "🚪 Confirm Student EXIT" : "✅ Confirm Student RETURN"}
          </button>
        </div>
      </div>
    </div>
